...
async def _check_analysis(db, analysis_id: int) -> list[CheckResult]:
    ...
    # 3. nlp_all_predictions: rows + pred_* fully populated (Bug 1 guard)
    ...
    # 4. sentence_prediction: 50 rows with non-empty context
    ...
    # 5. llm_pipeline_intermediate: rows for every domain with survived_filter signal
    ...
    # 7. patient_summary: exactly 1 row per filename (Bug 2 guard)
