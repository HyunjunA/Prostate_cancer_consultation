...
class TestGrantPatientAccess:
    """POST /api/auth/users/{id}/patients grants patient access."""

    async def test_grant_access_success(self, client, api_headers):
        """Should return 201 with the access record."""
        create_resp = await client.post(
            "/api/auth/users",
            headers=api_headers,
            json={"username": "accessuser", "role": "user"},
        )
        user_id = create_resp.json()["id"]

        resp = await client.post(
            f"/api/auth/users/{user_id}/patients",
            headers=api_headers,
            json={"patient_id": "PAT-001", "access_type": "read"},
        )
        assert resp.status_code == 201
        data = resp.json()
    ...
    async def test_grant_write_access(self, client, api_headers):
        """Should accept 'write' access_type."""
        create_resp = await client.post(
            "/api/auth/users",
            headers=api_headers,
            json={"username": "writeuser", "role": "user"},
        )
        user_id = create_resp.json()["id"]
        ...
        resp = await client.post(
            f"/api/auth/users/{user_id}/patients",
            headers=api_headers,
            json={"patient_id": "PAT-002", "access_type": "write"},
        )
        assert resp.status_code == 201
        assert resp.json()["access_type"] == "write"

    async def test_duplicate_access_returns_409(self, client, api_headers):
        """Granting access twice for the same patient should return 409."""
        create_resp = await client.post(
            "/api/auth/users",
            headers=api_headers,
            json={"username": "dupuser", "role": "user"},
        )
        user_id = create_resp.json()["id"]
    ...
    async def test_grant_access_nonexistent_user(self, client, api_headers):
        """Should return 404 for nonexistent user."""
        resp = await client.post(
            "/api/auth/users/99999/patients",
            headers=api_headers,
            json={"patient_id": "PAT-001"},
        )
        assert resp.status_code == 404
...
class TestListPatientAccess:
    """GET /api/auth/users/{id}/patients lists patient access records."""

    async def test_list_access_empty(self, client, api_headers):
        """Should return empty list when no access records exist."""
        create_resp = await client.post(
            "/api/auth/users",
            headers=api_headers,
            json={"username": "noaccess", "role": "user"},
        )
        user_id = create_resp.json()["id"]

        resp = await client.get(
            f"/api/auth/users/{user_id}/patients", headers=api_headers
        )
        assert resp.status_code == 200
        assert resp.json() == []

    async def test_list_access_after_grant(self, client, api_headers):
        """Should list granted access records."""
        create_resp = await client.post(
            "/api/auth/users",
            headers=api_headers,
            json={"username": "hasaccess", "role": "user"},
        )
        user_id = create_resp.json()["id"]
...
class TestRevokePatientAccess:
    """DELETE /api/auth/users/{id}/patients/{patient_id} revokes access."""

    async def test_revoke_access_success(self, client, api_headers):
        """Should return 204 on successful revocation."""
        create_resp = await client.post(
            "/api/auth/users",
            headers=api_headers,
            json={"username": "revokeaccess", "role": "user"},
        )
        user_id = create_resp.json()["id"]
        ...
        resp = await client.delete(
            f"/api/auth/users/{user_id}/patients/PAT-001", headers=api_headers
        )
        assert resp.status_code == 204

    async def test_revoke_nonexistent_access(self, client, api_headers):
        """Should return 404 when access record doesn't exist."""
        create_resp = await client.post(
            "/api/auth/users",
            headers=api_headers,
            json={"username": "nopat", "role": "user"},
        )
        user_id = create_resp.json()["id"]

        resp = await client.delete(
            f"/api/auth/users/{user_id}/patients/NONEXISTENT", headers=api_headers
        )
        assert resp.status_code == 404
