...
# Works without backend modifications even when question count or content changes
# """
...
# # ──────────────────────────────────────────────────────────────────────────────
# # Pydantic Model - Flexible structure
# # ──────────────────────────────────────────────────────────────────────────────
# class SurveySubmission(BaseModel):
#     """
#     Generic survey submission model
#     - survey_type: Survey type (baseline, sdm, dcs, risk_perception, satisfaction, questions)
#     - file: File identifier
#     - speaker: Patient/user identifier
#     - answers: Dynamic response data (accepts any structure)
#     - metadata: Additional metadata (optional)
#     """
#     survey_type: str
#     file: str
#     speaker: str
#     answers: Dict[str, Any]  # Flexible structure - accepts any question/answer format
#     metadata: Optional[Dict[str, Any]] = None


# # ──────────────────────────────────────────────────────────────────────────────
# # Endpoint - Handle all surveys with a single endpoint
# # ──────────────────────────────────────────────────────────────────────────────
# @router.post("/submit")
# async def submit_survey(submission: SurveySubmission):
#     """
#     Universal endpoint for all survey submissions
    
#     Expected survey_types:
#     - baseline: Demographic information
#     - sdm: Shared Decision Making (4 items)
#     - dcs: Decisional Conflict Scale (16 items)
#     - risk_perception: Risk Perception (5 items)
#     - satisfaction: Patient Satisfaction (4 items + feedback)
#     - questions: Patient Questions (10 items)
#     """
...
#     # ═══════════════════════════════════════════════════════════════════════
#     # REDCap Configuration from .env
#     # ═══════════════════════════════════════════════════════════════════════
#     REDCAP_API_URL = os.getenv("REDCAP_API_URL")
#     REDCAP_API_TOKEN = os.getenv("REDCAP_API_TOKEN")

#     print("REDCAP_API_URL:", REDCAP_API_URL)
#     print("REDCAP_API_TOKEN:", REDCAP_API_TOKEN)
    
#     timestamp = datetime.now().isoformat()
    
#     # ═══════════════════════════════════════════════════════════════════════
#     # Print all submission fields
#     # ═══════════════════════════════════════════════════════════════════════
#     print("\n" + "=" * 80)
...
#     print(json.dumps(submission.model_dump(), indent=2, ensure_ascii=False))
#     print("-" * 80)
#     print(f"📁 File: {submission.file}")
#     print(f"👤 Speaker: {submission.speaker}")
#     print(f"📝 Survey Type: {submission.survey_type.upper()}")
#     print("-" * 80)
#     print("📊 Answers:")
...
#         print("-" * 80)
#         print("ℹ️  Metadata:")
#         print(json.dumps(submission.metadata, indent=2, ensure_ascii=False))
...
# # ──────────────────────────────────────────────────────────────────────────────
# # Endpoint - Handle all surveys with a single endpoint and import to REDCap 
# # This is the more complex version that includes REDCap integration.
# # ──────────────────────────────────────────────────────────────────────────────
# # @router.post("/submit")
# # async def submit_survey(submission: SurveySubmission):
# #     """
# #     Universal endpoint for all survey submissions
# #     Receives survey data and imports to REDCap project
...
# #         print("❌ REDCap configuration missing in .env file")
# #         return {
# #             "status": "error",
# #             "message": "REDCap configuration missing",
# #             "survey_type": submission.survey_type,
# #             "file": submission.file,
# #             "speaker": submission.speaker
# #         }
...
# #     # ═══════════════════════════════════════════════════════════════════════
# #     # Print all submission fields (for debugging)
# #     # ═══════════════════════════════════════════════════════════════════════
# #     print("\n" + "=" * 80)
# #     print(f"📋 SURVEY SUBMISSION RECEIVED")
# #     print("=" * 80)
...
# #     # ═══════════════════════════════════════════════════════════════════════
# #     # Prepare REDCap record data
# #     # ═══════════════════════════════════════════════════════════════════════
    
# #     # Create record_id from file and speaker
# #     record_id = f"{submission.speaker}"  # Or use: f"{submission.file}_{submission.speaker}"
    
# #     # Map survey_type to REDCap instrument name
# #     instrument_map = {
...
# #     instrument_name = instrument_map.get(submission.survey_type, submission.survey_type)
    
# #     # Build REDCap record with prefixed field names
# #     redcap_record = {
# #         "record_id": record_id,
# #     }
    
# #     # Add prefixed fields based on survey type
# #     prefix = submission.survey_type
# #     for field_name, field_value in submission.answers.items():
# #         # Handle array values (convert to comma-separated string for REDCap)
# #         if isinstance(field_value, list):
# #             field_value = ", ".join(str(v) for v in field_value)
        
# #         # Add field with prefix: e.g., dcs_q1, baseline_sexAtBirth
# #         redcap_field_name = f"{prefix}_{field_name}"
# #         redcap_record[redcap_field_name] = field_value
    
# #     # Mark instrument as complete (0=Incomplete, 1=Unverified, 2=Complete)
# #     redcap_record[f"{instrument_name}_complete"] = "2"
    
# #     # Add timestamp field if exists in your REDCap project
# #     redcap_record[f"{prefix}_timestamp"] = timestamp
    
# #     print("-" * 80)
# #     print("📤 REDCap Record to Import:")
# #     print(json.dumps(redcap_record, indent=2, ensure_ascii=False))
# #     print("-" * 80)
    
# #     # ═══════════════════════════════════════════════════════════════════════
# #     # Import to REDCap
# #     # ═══════════════════════════════════════════════════════════════════════
# #     redcap_response = None
# #     redcap_error = None
...
# #         response = requests.post(REDCAP_API_URL, data=fields)
        
# #         print(f"🔗 REDCap API Response:")
# #         print(f"   HTTP Status: {response.status_code}")
# #         print(f"   Response: {response.text}")
        
# #         if response.status_code == 200:
# #             redcap_response = response.json() if response.text else {"status": "success"}
# #             print(f"✅ REDCap import successful!")
# #         else:
# #             redcap_error = response.text
# #             print(f"❌ REDCap import failed: {response.text}")
...
# #             "success": redcap_error is None,
# #             "response": redcap_response,
# #             "error": redcap_error
# #         }
# #     }
...
# # ──────────────────────────────────────────────────────────────────────────────
# # Convenience endpoints for each survey type (optional)
# # ──────────────────────────────────────────────────────────────────────────────
...
# ):
#     """Submit Baseline Info survey"""
#     return await submit_survey(SurveySubmission(
...
# @router.post("/dcs")
# async def submit_dcs(
#     file: str,
#     speaker: str,
#     answers: Dict[str, Any]
# ):
#     """Submit Decisional Conflict Scale survey"""
#     return await submit_survey(SurveySubmission(
#         survey_type="dcs",
#         file=file,
#         speaker=speaker,
...
# @router.post("/risk-perception")
# async def submit_risk_perception(
#     file: str,
#     speaker: str,
#     answers: Dict[str, Any]
# ):
...
#     answers: Dict[str, Any]
# ):
#     """Submit Patient Questions survey"""
#     return await submit_survey(SurveySubmission(
