...
// PATIENT QUESTIONS SECTION ADDED AT THE END
...
/* =============================================
   PASSIVE ENGAGEMENT TRACKING SYSTEM
...
interface TrackingEvent {
  eventType:
    ...
    | "button_click"
    | "section_view"
    | "question_answer"; // ⭐ NEW: for tracking question answers
  elementId: string;
  timestamp: string;
...
const useDwellTimeTracking = (
  ...
  const checkVisibility = useCallback(() => {
    ...
    console.log(
      `🔍 [Debug] Checking ${trackableElements.length} elements for dwell time`
    );
    ...
    trackableElements.forEach((element) => {
      ...
      if (isVisible) {
        if (!dwellStartTimes.current.has(elementId)) {
          ...
          console.log(
            `👁️ [Debug] Started tracking dwell time for: ${elementId}`
          );
        }
        ...
        if (
          ...
          console.log(
            `✅ [Debug] Dwell time threshold reached for: ${elementId} (${dwellTime}ms)`
          );
          trackingManager.recordEvent({
...
const StarRating: React.FC<{
  ...
  isDark?: boolean;
  trackingName?: string;
}> = ({ value, onChange, label, isDark, trackingName }) => {
  return (
...
const PatientReport: React.FC<PatientReportProps> = ({
  ...
  useEffect(() => {
    console.log(
      "🔍 Fetching filtered patient scoring for: quality-coded-nlp-pilot-sid-1.xlsx / Patient_quality-coded-nlp-pilot-sid-1"
    );
    fetchScoringFiltered(
  ...
  useEffect(() => {
    console.log("💬 Fetching all patient responses...");
    fetchResponsesAll();
  }, []);

  useEffect(() => {
    console.log(
      "🔍 Fetching filtered patient responses for: quality-coded-nlp-pilot-sid-1.xlsx / Patient_quality-coded-nlp-pilot-sid-1"
    );
    fetchResponsesFiltered(
      "quality-coded-nlp-pilot-sid-1.xlsx",
      "Patient_quality-coded-nlp-pilot-sid-1"
  ...
  /* ⭐ NEW: Patient Questions State */
  const [questionAnswers, setQuestionAnswers] = useState<{
    [key: string]: string;
  }>({});

  /* ⭐ NEW: Define 10 Questions */
  ...
  /* ⭐ NEW: Handle question answer changes with tracking */
    ...
    if (answer.length > 0) {
      trackingManager.recordEvent({
        ...
        metadata: {
          ...
          question: patientQuestions[questionIndex],
        },
      });
    }
  };

  /* ⭐ NEW: Submit all answers */
  ...
  /* ⭐ NEW: Check if current tab is "Patient Questions" */
        ...
        {viewMode === "topics" ? (
          ...
          >
            <div className="px-6 lg:px-12 py-12 grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-10">
              {/* MAIN COLUMN */}
              <div>
                {/* Overall Summary shown on landing */}
                {activeTab === null && (
                  <section
                ...
                {isQuestionsTab && (
                  <section data-track-proximity="PatientQuestions_Section">
                    <div className="mb-8">
                      ...
                      >
                        We'd love to hear from you. Please take a moment to
                        answer these questions to help us better understand your
                        needs.
                      </p>
                    </div>
                    ...
                    <div className="mt-10 flex justify-center">
                      <button
                        ...
                        data-track-proximity="Questions_Submit_Button"
                        className={cx(
              ...
              {/* ⭐ UPDATED: Right Sidebar Navigation with Questions */}
              <aside className={cx("lg:sticky lg:top-6 h-max no-print")}>
                ...
                >
                  ...
                  <div className="space-y-2">
                    ...
                    {/* ⭐ NEW: Questions Navigation Button */}
          ...
          >
            <div className="px-12 py-12">
              ...
              {Object.entries<any>(patientData.consultationTopics).map(
                ([topicName, topicData], index) => (
                  <div
                    key={topicName}
                    className="relative"
                    data-track-proximity={`FullMode_Topic_${topicName.replace(
                      /\s+/g,
                      ""
                    )}`}
                  >
                    ...
                    <div
                      data-track-proximity={`FullMode_Summary_${topicName.replace(
                        /\s+/g,
                        ""
                      )}`}
                      className={cx(
                    ...
                    <div className="mb-12">
                      <button
                        ...
                        data-track-proximity={`FullMode_ToggleKey_${topicName.replace(
                          /\s+/g,
                          ""
                        )}`}
                        className={cx(
                      ...
                      {showKeys[topicName] && (
                        <div
                          data-track-proximity={`FullMode_KeyStatements_${topicName.replace(
                            /\s+/g,
                            ""
                          )}`}
                        >
              ...
              {/* ⭐ NEW: Questions Section in Full Mode */}
              <div className="py-8">
                <div
                  className={cx(
                    "border-t-2",
              ...
              <section data-track-proximity="FullMode_PatientQuestions">
                <div className="mb-8">
                  <div className="flex items-center mb-3">
                    <div
                      className={cx(
                  ...
                  >
                    We'd love to hear from you. Please take a moment to answer
                    these questions to help us better understand your needs.
                  </p>
                </div>
                ...
                <div className="space-y-6">
                  {patientQuestions.map((question, index) => (
                    <div
                      key={index}
                      data-track-proximity={`FullMode_Question_${
                        index + 1
                      }_Input`}
                      className={cx(
                        "p-6 rounded-xl border transition-all",
                ...
                <div className="mt-10 flex justify-center">
                  <button
                    onClick={handleSubmitQuestions}
                    data-track-proximity="FullMode_Questions_Submit"
                    className={cx(
                      "px-8 py-4 rounded-lg text-lg font-semibold transition-all shadow-lg hover:shadow-xl",
                      isDarkMode
                        ? "bg-green-700 text-green-100 hover:bg-green-600"
