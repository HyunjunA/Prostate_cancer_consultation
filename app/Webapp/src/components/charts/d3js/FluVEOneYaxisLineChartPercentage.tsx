...
interface DataPoint {
  week_start: string;
  week_ending: string;
  percent_positive_cdc: number;
  percent_positive_twitter_x: number;
...
export const FluVEOneYaxisLineChartPercentage = ({
  data,
  width = 800,
  height = 400,
  title,
  ...
  const getStateKey = (label) => {
    if (label === "CDC") return "cdc";
    if (label === "X (Twitter)") return "twitterx";
  ...
  useEffect(() => {
    ...
    d3.select("body").selectAll(`.tooltip-per-${id}`).remove();

    const margin = { top: 30, right: 175, bottom: 110, left: 80 };
    ...
    const validData = data.filter(
      (d) =>
        d.percent_positive_cdc !== -1 || d.percent_positive_twitter_x !== -1
    );
    ...
    const yScale = d3
      ...
      .domain([
        ...
        (d3.max(data, (d) =>
          Math.max(
            d.percent_positive_cdc === -1 ? 0 : d.percent_positive_cdc,
            d.percent_positive_twitter_x === -1
              ? 0
              : d.percent_positive_twitter_x
          )
        ) as number) * 1.1,
      ])
      .range([innerHeight, 0]);
    ...
    const createLine = d3
      ...
      .y((d) => yScale(d.percent_positive_cdc));

    const cdcSegments = splitIntoSegments(data, (d) => d.percent_positive_cdc);
    const twitterxSegments = splitIntoSegments(
      data,
      (d) => d.percent_positive_twitter_x
    );

    cdcSegments.forEach((segment) => {
      g.append("path")
        .datum(segment)
        .attr("fill", "none")
        .attr("stroke", colors.lines.cdc)
        .attr("stroke-width", 3)
        .attr("class", "percent_positive_cdc-line")
        .attr("d", createLine)
        .style("opacity", visibleLines.cdc ? 1 : 0);

      g.selectAll(`.dot-cdc-${segment[0].week_ending}`)
        .data(segment)
        .enter()
        .append("circle")
        .attr("class", "dot-cdc")
        .attr("cx", (d) => xScale(d.week_ending) || 0)
        .attr("cy", (d) => yScale(d.percent_positive_cdc))
        .attr("r", 4)
        .style("fill", colors.lines.cdc)
        .style("opacity", visibleLines.cdc ? 1 : 0);
    });

    const twitterxLine = d3
      .line<DataPoint>()
      .x((d) => xScale(d.week_ending) || 0)
      .y((d) => yScale(d.percent_positive_twitter_x));

    twitterxSegments.forEach((segment) => {
      // 기존 twitterx line 코드는 그대로 유지
      g.append("path")
        .datum(segment)
        .attr("fill", "none")
        .attr("stroke", colors.lines.twitterx)
        .attr("stroke-width", 3)
        .attr("class", "percent_positive_twitter_x-line")
        .attr("d", twitterxLine)
        .style("opacity", visibleLines.twitterx ? 1 : 0);
      ...
      g.selectAll(`.dot-twitterx-${segment[0].week_ending}`)
        .data(segment)
        .enter()
        .append("circle")
        .attr("class", "dot-twitterx")
        .attr("cx", (d) => xScale(d.week_ending) || 0)
        .attr("cy", (d) => yScale(d.percent_positive_twitter_x))
        .attr("r", 4)
        .style("fill", colors.lines.twitterx)
        .style("opacity", visibleLines.twitterx ? 1 : 0);
    ...
    g.append("text")
      ...
      .attr("x", innerWidth * 0.95)
      .attr("y", innerHeight + margin.bottom - 120)
    ...
    svg
      .append("text")
      .attr("id", "zoom-instructions-per")
      .attr("x", width / 2)
      .attr("y", margin.top / 2 + 50) // Adjust the y position to appear below the title
      .attr("text-anchor", "middle")
    ...
    svg
      ...
      .style("font-style", "italic") // This makes the text italic
      .style("fill", colors.text)
      .text("Percent Positive: Our Data vs CDC Over Time");
    ...
    const legend = svg
      ...
      .attr(
        "transform",
        `translate(${width - margin.right + 35},${margin.top + 0})`
      );
    ...
    const legendItems = [
      {
        label: "CDC",
        color: colors.lines.cdc,
        class: "percent_positive_cdc-line",
      },
      {
        label: "X (Twitter)",
        color: colors.lines.twitterx,
        class: "percent_positive_twitter_x-line",
      },
      // {
    ...
    legendItems.forEach((item, i) => {
      ...
      const checkmark = legendGroup
        ...
        .style(
          "visibility",
          visibleLines[getStateKey(item.label)] ? "visible" : "hidden"
        );
      ...
      legendGroup
        ...
        .on("click", () => {
          ...
          console.log("key-clicked-visibleLines", visibleLines);
          const stateKey = key === "cdc" ? "cdc" : "twitterx";
          ...
          console.log("key-clicked-newVisibility", newVisibility);
          ...
          g.selectAll(`.dot-${stateKey}`)
            .style("opacity", newVisibility ? 1 : 0)
            .style("pointer-events", newVisibility ? "auto" : "none");

          // hover point 표시/숨김 처리
          if (stateKey === "cdc") {
            cdcPoint.style("display", newVisibility ? "block" : "none");
          } else if (stateKey === "twitterx") {
            twitterxPoint.style("display", newVisibility ? "block" : "none");
          }
        })
        .on("mouseover", () => {
          const key = item.label.toLowerCase();
          ...
          g.selectAll(
            ".percent_positive_twitter_x-line, .percent_positive_cdc-line"
          ).style("opacity", function () {
            const lineKey = d3
              .select(this)
              .classed("percent_positive_twitter_x-line")
              ? "twitterx"
              : "cdc";
            return visibleLines[lineKey] ? 0.2 : 0;
          });

          g.selectAll(".dot-twitterx, .dot-cdc")
            .style("opacity", function () {
              const dotKey = d3.select(this).classed("dot-twitterx")
                ? "twitterx"
                : "cdc";
          ...
          if (visibleLines[stateKey]) {
            g.selectAll(`.${item.class}`)
              .style("opacity", 1)
              .style("stroke-width", 5);
            g.selectAll(`.dot-${stateKey}`).style("opacity", 1).attr("r", 6);
          }
    ...
    const tooltipDiv = d3
      .select("body")
      .append("div")
      .attr("class", `tooltip-per-${id}`)
      .attr("data-per-chart-id", id)
    ...
    const selectionGroup = g.append("g").attr("class", "selection-group-per");
    const brushGroup = g.append("g").attr("class", "brush-group-per");
    ...
    const cdcPoint = focus
      .append("circle")
      .attr("r", 4)
      .style("fill", colors.lines.cdc);

    const twitterxPoint = focus
      .append("circle")
      .attr("r", 4)
      .style("fill", colors.lines.twitterx);
    ...
    const updateSelection = () => {
      ...
      if (selectedDateOnXaxis) {
        ...
        // const y = yScale(tempD?.percent_positive_cdc);
        ...
        if (tempD?.percent_positive_cdc !== -1) {
          selectionGroup
            .append("circle")
            .attr("cx", x)
            .attr("cy", yScale(tempD?.percent_positive_cdc))
            .attr("r", 6)
            .style("fill", colors.lines.cdc);
        }

        if (tempD?.percent_positive_twitter_x !== -1) {
          selectionGroup
            .append("circle")
            .attr("cx", x)
            .attr("cy", yScale(tempD?.percent_positive_twitter_x))
            .attr("r", 6)
            .style("fill", colors.lines.twitterx);
        }
  ...
  <div style="color: ${colors.lines.cdc}; font-size: ${
          chartStyles.tooltip.fontSize
        }">
    CDC: ${
      tempD?.percent_positive_cdc !== -1
        ? `${tempD?.percent_positive_cdc} %`
        : `<span style="font-style: italic; color: ${colors.tooltip.text}">Data not available for this period</span>`
    }
  </div>
  <div style="color: ${colors.lines.twitterx}; font-size: ${
          chartStyles.tooltip.fontSize
        }">
    X (Twitter): ${
      tempD?.percent_positive_twitter_x !== -1
        ? `${tempD?.percent_positive_twitter_x} %`
        : `<span style="font-style: italic; color: ${colors.tooltip.text}">Data not available for this period</span>`
    }
  </div>
      `;
      ...
      .on("mousemove", function (event) {
        ...
        if (xPos >= 0 && xPos <= innerWidth) {
          ...
          if (d) {
            ...
            if (
              d.percent_positive_cdc !== -1 &&
              yScale(d.percent_positive_cdc) >= 0 &&
              yScale(d.percent_positive_cdc) <= innerHeight
            ) {
              cdcPoint
                .style("display", "block")
                .attr("cx", x)
                .attr("cy", yScale(d.percent_positive_cdc))
                .attr("r", 6);
            } else {
              cdcPoint.style("display", "none");
            }

            if (
              d.percent_positive_twitter_x !== -1 &&
              yScale(d.percent_positive_twitter_x) >= 0 &&
              yScale(d.percent_positive_twitter_x) <= innerHeight
            ) {
              twitterxPoint
                .style("display", "block")
                .attr("cx", x)
                .attr("cy", yScale(d.percent_positive_twitter_x))
                .attr("r", 6);
            } else {
              twitterxPoint.style("display", "none");
            }
...
export default FluVEOneYaxisLineChartPercentage;
