...
/**
 * PatientInitialVisitReportV42.tsx
 *
 * [V42] Copy of V41 with ONE change: the evidence ("relevant sentences")
 * display. V41 gave each treatment category (surgery / radiation / ablation /
 * <missing>) its OWN "View relevant sentences" dropdown, so a multi-treatment
 * side-effect domain (ED etc.) showed several dropdowns. V42 collapses these
 * into a SINGLE domain-level dropdown; inside it, each category is a labelled
 * section (treatment badge, or a note for <missing>, or no label for cp/le), so
 * the categories stay distinguishable while opening together under one toggle.
 * not-discussed (every category ai_score === 0) disables the single toggle.
...
const TopicCard = React.forwardRef<TopicCardHandle, TopicCardProps>(({
  ...
  // [V42] Every domain now uses ONE "View relevant sentences" dropdown. Inside
  // it, each treatment category (surgery / radiation / ablation / <missing>) is
  // a labelled section instead of getting its own separate dropdown (V41's
  // per-treatment layout). This keeps categories distinguishable while showing
  // them together under a single toggle.
  //
  // "not discussed" now means EVERY treatment category was scored 0 (whatever
  // the category count), which disables the single toggle. Categories with a
  // real/absent score (!== 0) are the ones shown, grouped and labelled.
  const allNotDiscussed =
    (aiSubDomains?.length ?? 0) >= 1 &&
    (aiSubDomains ?? []).every((s) => s.aiScore === 0);
  const evidenceCategories = (aiSubDomains ?? []).filter(
    (s) => s.aiScore !== 0,
  );
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* [V42] Unified evidence dropdown. One domain-level toggle for
              every domain; when open, each treatment category (surgery /
              radiation / <missing> note / cp-le = no label) is a labelled
              section. Replaces V41's per-treatment dropdowns so a domain shows a
              single "View relevant sentences" toggle with the categories grouped
              and labelled inside it. */}
          <div className="mb-6">
            <button
              type="button"
              disabled={allNotDiscussed}
              onClick={allNotDiscussed ? undefined : onToggleEvidence}
              aria-disabled={allNotDiscussed}
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span>
                  {showEvidence ? "Hide" : "View"} relevant sentences from your
                  visit
                </span>
              </div>
              {allNotDiscussed ? null : showEvidence ? (
                <ChevronUp size={16} />
              ) : (
                <ChevronDown size={16} />
              )}
            ...
            {allNotDiscussed ? (
              ...
              >
                ...
                {evidenceCategories.length > 0 ? (
                  evidenceCategories.map((cat, cidx) => {
                    // Real treatment -> label badge. "<missing>" -> the AI found
                    // a relevant sentence but could not tie it to a specific
                    // treatment; show an explanatory note. null (cp/le) -> no
                    // label at all.
                    const treatmentLabel =
                      cat.treatment && cat.treatment !== "<missing>"
                        ? cat.treatment
                        : null;
                    ...
                    return (
                      <div key={cidx} className="space-y-2">
                        {treatmentLabel ? (
                          <p
                            className={cx(
                              "text-xs font-bold uppercase tracking-wide px-1",
                  ...
                  // Fallback: no grouped sub-domains, but flat sources exist
                  // (e.g. cp/le when the domain carries no treatment grouping).
                  ...
                  >
                    No relevant sentences from your visit were found for this
                    topic.
                  </div>
                )}
              </div>
  ...
  const {
    ...
    if (aiSummaryData?.domains) {
      ...
      const DESIGNATED_TREATMENT: Record<string, string> = {
        ...
        "Irritative Urinary Symptoms": "radiation",
      };
      ...
      <div className="w-full flex-1 max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-12" id="report-content">
        ...
        */}
        ...
        {(() => {
          ...
          // Combined-survey flow: on the last domain the Next button becomes a
          // "Continue" action that hands off to the follow-up surveys via
          // onComplete(). Standalone (onComplete undefined) keeps Next disabled
          // on the last screen.
...
export default PatientReportFirstVisitV41;
