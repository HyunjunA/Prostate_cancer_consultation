...
// ✅ COMPLETE PASSIVE ENGAGEMENT TRACKING SYSTEM INCLUDED
// All tracking logic is self-contained in this file
var react_1 = require("react");
var XLSX = require("xlsx");
// Tracking Event Manager (Singleton)
var TrackingEventManager = /** @class */ (function () {
    function TrackingEventManager() {
        this.events = [];
        this.listeners = [];
    }
    TrackingEventManager.prototype.recordEvent = function (event) {
        this.events.push(event);
        console.log("\uD83D\uDCCA [Tracking Event]", event);
        this.listeners.forEach(function (listener) { return listener(event); });
    };
    TrackingEventManager.prototype.subscribe = function (listener) {
        var _this = this;
    ...
    TrackingEventManager.prototype.getEvents = function () {
        return __spreadArrays(this.events);
    };
    TrackingEventManager.prototype.getEventsByType = function (type) {
        return this.events.filter(function (e) { return e.eventType === type; });
    };
    TrackingEventManager.prototype.clear = function () {
        this.events = [];
    };
    TrackingEventManager.prototype.exportEvents = function () {
        return JSON.stringify(this.events, null, 2);
    };
    return TrackingEventManager;
}());
var trackingManager = new TrackingEventManager();
// Make trackingManager accessible from browser console
if (typeof window !== "undefined") {
    window.trackingManager = trackingManager;
}
// Cursor Proximity Tracking Hook
var useCursorProximityTracking = function (config) {
    if (config === void 0) { config = { threshold: 150, debounceMs: 100 }; }
    var proximityStates = react_1.useRef(new Map());
    var debounceTimerRef = react_1.useRef(null);
    var calculateDistance = react_1.useCallback(function (mouseX, mouseY, element) {
    ...
    var handleMouseMove = react_1.useCallback(function (e) {
        ...
        debounceTimerRef.current = setTimeout(function () {
            var trackableElements = document.querySelectorAll("[data-track-proximity]");
            trackableElements.forEach(function (element) {
                var htmlElement = element;
                var elementId = htmlElement.getAttribute("data-track-proximity");
                if (!elementId)
                    return;
                var distance = calculateDistance(e.clientX, e.clientY, htmlElement);
                var isNear = distance <= config.threshold;
                var wasNear = proximityStates.current.get(elementId) || false;
                if (isNear && !wasNear) {
...
var useScrollDepthTracking = function (config) {
    if (config === void 0) { config = { thresholds: [25, 50, 75, 100], debounceMs: 200 }; }
    var triggeredThresholds = react_1.useRef(new Set());
    var debounceTimerRef = react_1.useRef(null);
    var handleScroll = react_1.useCallback(function () {
        ...
        debounceTimerRef.current = setTimeout(function () {
            ...
            var scrollPercentage = scrollableHeight > 0 ? (scrollTop / scrollableHeight) * 100 : 100;
    ...
    react_1.useEffect(function () {
        console.log("🔍 [Debug] Scroll tracking initialized");
...
// Dwell Time Tracking Hook
var useDwellTimeTracking = function (config) {
    if (config === void 0) { config = { minDwellTime: 2000, trackingInterval: 500 }; }
    var dwellTimers = react_1.useRef(new Map());
    var dwellStartTimes = react_1.useRef(new Map());
    var intervalRef = react_1.useRef(null);
    var checkVisibility = react_1.useCallback(function () {
        var trackableElements = document.querySelectorAll("[data-track-proximity]");
        console.log("\uD83D\uDD0D [Debug] Checking " + trackableElements.length + " elements for dwell time");
        trackableElements.forEach(function (element) {
            var htmlElement = element;
            var elementId = htmlElement.getAttribute("data-track-proximity");
            if (!elementId)
            ...
            var isVisible = rect.top < window.innerHeight &&
                rect.bottom > 0 &&
            ...
            if (isVisible) {
                if (!dwellStartTimes.current.has(elementId)) {
                    ...
                    console.log("\uD83D\uDC41\uFE0F [Debug] Started tracking dwell time for: " + elementId);
                }
                var startTime = dwellStartTimes.current.get(elementId);
                var dwellTime = currentTime - startTime;
                var previousDwellTime = dwellTimers.current.get(elementId) || 0;
                dwellTimers.current.set(elementId, dwellTime);
                if (dwellTime >= config.minDwellTime &&
                    previousDwellTime < config.minDwellTime) {
                    console.log("\u2705 [Debug] Dwell time threshold reached for: " + elementId + " (" + dwellTime + "ms)");
                    trackingManager.recordEvent({
    ...
    react_1.useEffect(function () {
        console.log("🔍 [Debug] Dwell time tracking initialized");
...
// Combined Passive Tracking Hook
var usePassiveTracking = function (config) {
    useCursorProximityTracking(config === null || config === void 0 ? void 0 : config.proximity);
    useScrollDepthTracking(config === null || config === void 0 ? void 0 : config.scrollDepth);
    useDwellTimeTracking(config === null || config === void 0 ? void 0 : config.dwellTime);
    return { trackingManager: trackingManager };
};
/* ---------------------------------------------
   SMALL UTILS
---------------------------------------------- */
...
var StarRating = function (_a) {
    ...
    return (react_1["default"].createElement("div", { className: "flex items-center gap-3" },
        ...
        react_1["default"].createElement("div", { className: "flex items-center gap-1" }, [1, 2, 3, 4, 5].map(function (i) { return (react_1["default"].createElement("button", { key: i, type: "button", "aria-label": "Rate " + i, onClick: function () {
                onChange(i);
                // Record rating event with trackingManager
                trackingManager.recordEvent({
            ...
            }, "data-track-proximity": trackingName ? trackingName + "_Star" + i : undefined, className: cx("w-8 h-8 rounded-full grid place-items-center border transition", isDark
...
var PatientReport = function (_a) {
    ...
    /* ---------------------------------------------
       ⭐ PASSIVE TRACKING: Initialize all tracking systems
    ---------------------------------------------- */
    usePassiveTracking({
