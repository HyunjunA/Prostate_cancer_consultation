  ...
  return (
    <div data-track-proximity="SDM_Survey">
      ...
      <div className="space-y-4">
        {SDM_QUESTIONS.map((q, idx) =>
          q.type === "yesno" ? (
            <YesNoQuestion
              key={q.id}
              questionNumber={idx + 1}
              questionText={formatQuestion(q.text)}
              value={answers[q.id as keyof SDMAnswers] as YesNoAnswer}
              onChange={(v) => onChange(q.id as keyof SDMAnswers, v)}
              isDark={isDark}
              trackingName={`SDM_${q.id}`}
            ...
            <ScaleQuestion
              ...
              questionNumber={idx + 1}
              questionText={formatQuestion(q.text)}
              value={answers[q.id as keyof SDMAnswers] as ScaleAnswer}
              onChange={(v) => onChange(q.id as keyof SDMAnswers, v)}
              isDark={isDark}
              trackingName={`SDM_${q.id}`}
              onTrackEvent={onTrackEvent}
