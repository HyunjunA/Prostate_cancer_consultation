      ...
      <div className="space-y-6">
        ...
        {RISK_QUESTIONS.map((q, idx) => (
          <RiskQuestion
            ...
            options={q.options}
            value={
              answers[q.id as keyof RiskPerceptionAnswers] as string | null
            }
            onChange={(v) => onChange(q.id as keyof RiskPerceptionAnswers, v)}
            isDark={isDark}
            trackingName={`RiskPerception_${q.id}`}
            onTrackEvent={onTrackEvent}
          />
        ))}
      </div>
