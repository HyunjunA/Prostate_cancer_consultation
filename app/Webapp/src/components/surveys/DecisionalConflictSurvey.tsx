...
> = ({
  ...
  return (
    <div data-track-proximity="DCS_Survey">
      ...
      <QuestionSection isDark={isDark}>
        {DCS_QUESTIONS.slice(0, 5).map((q, idx) => (
          <LikertQuestion
            key={q.id}
            questionNumber={idx + 1}
            questionText={q.text}
            value={answers[q.id as keyof DecisionalConflictAnswers]}
            onChange={(v) =>
              onChange(q.id as keyof DecisionalConflictAnswers, v)
            }
            isDark={isDark}
            trackingName={`DCS_${q.id}`}
      ...
      <QuestionSection isDark={isDark}>
        {DCS_QUESTIONS.slice(5, 11).map((q, idx) => (
          <LikertQuestion
            key={q.id}
            questionNumber={idx + 6}
            questionText={q.text}
            value={answers[q.id as keyof DecisionalConflictAnswers]}
            onChange={(v) =>
              onChange(q.id as keyof DecisionalConflictAnswers, v)
      ...
      <QuestionSection isDark={isDark}>
        {DCS_QUESTIONS.slice(11, 16).map((q, idx) => (
          <LikertQuestion
            key={q.id}
            questionNumber={idx + 12}
            questionText={q.text}
            value={answers[q.id as keyof DecisionalConflictAnswers]}
            onChange={(v) =>
              onChange(q.id as keyof DecisionalConflictAnswers, v)
      ...
      {onSubmit && (
        <div className="mt-10 flex justify-center">
          ...
          >
            {isComplete
              ? "Submit Responses"
              : `Complete all ${totalQuestions} questions`}
          </button>
        </div>
      )}
    </div>
