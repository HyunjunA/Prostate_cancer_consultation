...
// generateSampleData() removed - replaced with API data
// Re-write functionality with API integration (sentence selection, DB save, rewrite history)
// Immediate UI update after modification (always reference latest data from topicsData)
// NEW: Score Summary API integration for GridView
// NEW: scoreSentence API integration (replaces mock rescoreSentence)
// FIX: showRewrite state moved to parent to prevent reset on re-render
...
const PhysicianReports: React.FC<PhysicianReportsProps> = ({
  ...
  // ═══════════════════════════════════════════════════════════
  // useDoctorData hook
  // NEW: Added scoreSentence for API-based scoring
  // ═══════════════════════════════════════════════════════════
  const {
    files,
    sentences,
  ...
  // FIX: Moved from DetailView to parent to prevent state reset on re-render
  ...
  const getScoreColorForValue = (score: number | null) => {
    if (score === null) {
      return isDarkMode
    ...
    return getScoreColor(roundedScore);
  };
  ...
  const titleByScore = (score: number | null) => {
  ...
  // ═══════════════════════════════════════════════════════════
  // GridView
  // FIX: Removed useMemo inside nested component
  // ═══════════════════════════════════════════════════════════
  const GridView = () => {
    ...
    // FIX: Direct calculation instead of useMemo
    ...
    const handleSuggestionClick = (
      ...
      // FIX: Reset sentence index when navigating to a new topic
      setSelectedSentenceIdx(0);
      // FIX: Open rewrite panel when suggestion is clicked
      setShowRewrite(true);
                      ...
                      <td className="px-6 py-6">
                        <button
                          onClick={() => {
                            ...
                            // FIX: Reset sentence index when navigating to a new topic
                            setSelectedSentenceIdx(0);
                            // FIX: Reset showRewrite when navigating without suggestion
                        ...
                        {scoreSummaryLoading ? (
                          ...
                          <span
                            className={cx(
                              ...
                              getScoreColorForValue(data.score)
                            )}
                          >
  ...
  // ═══════════════════════════════════════════════════════════
  // DetailView
  // FIX: selectedSentenceIdx and showRewrite moved to parent component
  // NEW: Uses scoreSentence API instead of mock rescoreSentence
  // ═══════════════════════════════════════════════════════════
  const DetailView = () => {
    // FIX: Removed local state - now using parent's selectedSentenceIdx and showRewrite
    ...
    // NEW: Updated handleSaveRewrite to use scoreSentence API
    const handleSaveRewrite = async () => {
      ...
      try {
        ...
        // NEW: Call scoreSentence API instead of mock function
        const scoreResult = await scoreSentence(newSentence, classNumber);
        const newScore = scoreResult?.score ?? 5; // Fallback to 5 if API fails
      ...
      <div className="space-y-8">
        ...
        <button
          onClick={() => {
            ...
            setNewSentence("");
            setSaveStatus({ status: "idle", message: "" });
            // FIX: Reset sentence index when navigating back
            setSelectedSentenceIdx(0);
          }}
          className={cx(
