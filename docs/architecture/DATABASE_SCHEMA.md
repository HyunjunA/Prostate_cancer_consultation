...
PostgreSQL schema for the Prostate Cancer Consultation Dashboard. **17 application tables** in the `public` schema (plus `alembic_version` for migration tracking). Schema lives in `app/Backend/models.py` (SQLAlchemy ORM) and is bootstrapped via `app/Backend/database_schema.sql` + Alembic migrations 001–009.
