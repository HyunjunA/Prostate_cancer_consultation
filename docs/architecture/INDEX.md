...
> Korean mirror: `INDEX_KR.md`. See each doc / `models.py` for detail. (Current schema: **15 app tables**
> + `alembic_version`, alembic head **029**.)
...
|---|---|---|
| `DATABASE_SCHEMA.md` / `_KR` | EN/KR | **Table catalog** — each of the 15 tables: purpose · why it exists · the anchor (parent–child) pattern |
...
## 2. Current DB — 15 app tables (by group)
...
**One-liner:** all patient survey answers live in one place (`patient_survey_submission_log`), behavior logs in `*_page_behavior`/`doctor_behavior`, pipeline outputs under `transcript_analysis_log`'s children — 15 app tables total.
